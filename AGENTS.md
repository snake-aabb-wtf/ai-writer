...
- 模型客户端必须检查 `choices[0].finish_reason`；`length`/`max_tokens` 表示输出截断，必须抛出可识别错误，让队列重试，不能把不完整 JSON 当作成功结果。
- 结构化设定/规划任务使用 12000 token 输出预算，章节任务使用 24000 token 输出预算；章节正文由提示词和解析器共同限制在 3500 个字符以内，避免思考 token 挤压 JSON 或正文无限增长。
...
- 默认模型任务超时为 300000ms（5 分钟），因为兼容模型可能把 reasoning token 计入输出预算；短任务可以显式设置更小的超时，但章节生成不能恢复为 120000ms。
