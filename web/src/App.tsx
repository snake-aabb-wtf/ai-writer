...
type Project = { id: string; name: string; language: string; genre: string; style: string; premise: string; status: string; currentStage: string; updatedAt: string };
...
const statusLabels: Record<string, string> = { draft: "草稿", running: "运行中", paused: "已暂停", completed: "已完成", error: "需处理" };
...
export default function App() {
  ...
  const loadProjects = useCallback(async (quiet = false) => {
    ...
    try {
      const [projectData, health] = await Promise.all([request<{ projects: Project[] }>("/api/projects"), request<{ modelConfigured: boolean }>("/health")]);
  ...
  return <div className="app-shell">
    <aside className="sidebar" aria-label="故事库导航">
      ...
      <div className="sidebar-rule" /><div className="eyebrow">你的故事库 <span>{projects.length.toString().padStart(2, "0")}</span></div>
      <div className="project-list">{projects.map((project) => <button className={`project-item ${project.id === selectedId ? "is-selected" : ""}`} key={project.id} onClick={() => setSelectedId(project.id)}><span className={dotClass(project.status)} /><span className="project-item-copy"><strong>{project.name}</strong><small>{project.genre} · {statusLabels[project.status]}</small></span><span className="project-arrow">↗</span></button>)}{!projects.length && <div className="empty-sidebar">还没有故事。<br />从一个念头开始。</div>}</div>
    ...
    <main className="main-canvas">
      <header className="topbar"><div className="breadcrumb"><span>工作台</span><i>/</i><strong>{activeProject?.name ?? "故事库"}</strong></div><div className="topbar-actions"><span className="live-indicator"><span /> {detail?.project.status === "running" ? "实时同步中" : "状态已保存"}</span><button className="icon-button" title="刷新" onClick={() => { void loadProjects(); void loadDetail(); }}>↻</button></div></header>
      {error && <div className="error-banner"><span>!</span>{error}<button onClick={() => setError(undefined)}>×</button></div>}
      {loading && !projects.length ? <LoadingState /> : activeProject && detail ? <Workspace detail={detail} onAction={runAction} /> : <WelcomeState onCreate={() => setShowCreate(true)} />}
    </main>
...
function Workspace({ detail, onAction }: { detail: Detail; onAction: (action: "generate" | "pause" | "resume") => void }) {
